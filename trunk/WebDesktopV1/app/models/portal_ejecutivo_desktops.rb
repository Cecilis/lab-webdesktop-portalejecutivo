class Portal_ejecutivo_desktops < ActiveRecord::Base
  
  def ContarHijos(tipo,padreid)
   @son = Portal_ejecutivo_desktops.count(:all, :select => 'count(*) ', :conditions => "tipo=#{tipo} and padre_id=#{padreid}" )
   return @son
  end
  ...
  def ObtenerHijos(tipo,padreid)
      ...
      if totaldeRegistros>0
        @tira = @tira+"  children: [ "
        @arbols = Portal_ejecutivo_desktops.find(:all, :conditions => "tipo=#{tipo} and padre_id=#{padreid}" )
        j=0
		...
		def BuscarTodosArbolJson(tipo)
			...
			@tira='[ '
   if totaldeRegistros>0 
				@arbols = Portal_ejecutivo_desktops.find(:all, :conditions => "tipo=#{tipo} and padre_id=0" )
				j=0
				@arbols.each do |arbol|
